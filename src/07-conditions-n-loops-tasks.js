...
// The intersection width is calculated by finding the overlap along the x-axis
// (rightmost left edge minus the leftmost right edge). If there's no overlap,
// the result will be 0 or negative, so we return the max value between 0 and the result.
// The intersection height is calculated by finding the overlap along the y-axis
// (bottommost top edge minus the topmost bottom edge). Similarly, if there's no overlap,
// the result will be 0 or negative, so we return the max value between 0 and the result.

  const condition1 = rect1.left + rect1.width <= rect2.left;
  const condition2 = rect2.left + rect2.width <= rect1.left;
  const condition3 = rect1.top + rect1.height <= rect2.top;
  const condition4 = rect2.top + rect2.height <= rect1.top;
...
function isInsideCircle(circle, point) {
  ...
  if (distance < circle.radius) {
    return true; // Point is strictly inside the circle
  }
  return false; // Point is either on the boundary or outside the circle
}
...
function findFirstSingleChar(str) {
  ...
  const firstChar = strToArray.find((element) => {
    const filteredArray = strToArray.filter((item) => item === element);
